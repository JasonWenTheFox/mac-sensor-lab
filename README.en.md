...
The Wi-Fi Radio provider uses public CoreWLAN getters for power/service/association state, RSSI, noise, derived SNR, channel, width, band, PHY, security mode, negotiated transmit rate, and transmit power. Error sentinels and unknown enum values are omitted. It reads no network name, access-point address, MAC/IP address, interface name, country code, or location; it performs no nearby-network scan and requests no Location permission. Negotiated PHY rate is not application throughput.
