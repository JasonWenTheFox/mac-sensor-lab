...
1. **Raw facts:** 24 independent providers covering non-unique Mac model class, SoC, CPU topology, unified memory, Metal GPU and Touch ID capabilities; per-display modes, gamut, EDR, and estimated geometry; system-disk media properties and NVMe SMART scalars; plus system performance, aggregate network/disk activity, battery/power, thermal pressure, read-only SMC, best-effort SPU ambient-light/motion reports, and lid angle.
...
Physical display dimensions may be CoreGraphics' 72-DPI estimate when EDID is unavailable, so dimensions, diagonal, and PPI always remain `Estimated`. Storage hardware includes only fixed categories and bounded numeric facts from the whole disk associated with the system volume and does not infer SMART from Disk Arbitration metadata. A separate NVMe SMART provider calls only Apple's public `SMARTReadData` API to show warning bits, composite temperature, spare capacity, and the controller's lifetime-use estimate. It excludes device model, serial, UUID, volume name, paths, identify data, and detailed error logs, and it does not collapse those fields into a single “health score.”
