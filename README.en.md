...
> **0.2.0 source preview:** the repository is ready for review, cloning, and local builds, but there is no notarized end-user download yet. The locally assembled app is ad-hoc signed for development verification. A public binary still needs Developer ID signing, notarization, and broader hardware testing.
...
1. **Raw facts:** 15 independent providers covering system performance, GPU, aggregate network/disk activity, battery/power, thermal pressure, display, storage, read-only SMC, best-effort SPU ambient-light/motion reports, and lid angle.
2. **Human-readable views:** status, units, source, timestamps, Raw/Derived/Estimated/Calibrated labels, bounded history, search, JSON/CSV snapshots, and size-limited continuous recording.
...
- Detailed implementation status and boundaries are in [docs/05-当前实现与后续路线.md](docs/05-当前实现与后续路线.md); the anonymous compatibility workflow is in [docs/06-匿名兼容性贡献指南.md](docs/06-匿名兼容性贡献指南.md).
