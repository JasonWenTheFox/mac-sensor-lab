...
2. **Human-readable views:** a dedicated Hardware Inventory screen with expandable fact cards plus user-started, page-local USB and local-camera capability inventories, along with status, units, source, hardware domain, access level, compatibility evidence, five-stage readiness, Raw/Derived/Estimated/Calibrated labels, bounded history, search, and versioned JSON/CSV exports.
3. **Experiments:** a session-only physical display ruler, a Force Touch normalized-pressure/stage visualizer, Wi-Fi signal/noise trends, a user-started nearby Wi-Fi channel snapshot, Sound Input Check, and Camera Check, plus level/motion trends, lid protractor, referenced light calibration, a four-channel ambient spectral fingerprint, battery and thermal trends, component thermals, internal power, and network/disk activity.
...
USB Device Tree is isolated from providers and exports. It performs one public-IOKit read only after a user action in Hardware Inventory and keeps only bounded VID/PID, `bcdDevice`, device/interface class triples, current configuration, connection-speed category, alternate setting, and current parent edges. It processes at most 128 devices, 512 interfaces, and 32 parent levels. The UI uses snapshot-local ordinals and clears when the page is left. It reads no serial/container/location/path/name/descriptor/power data, creates no USB user client, and sends no request, transfer, configuration, or reset. Connection speed is neither a device maximum nor measured throughput.

Camera Capabilities is likewise isolated from providers and exports. Only a user action in Hardware Inventory starts one public AVFoundation discovery of the format capabilities declared by built-in or external local cameras. It shows only snapshot-local ordinals, fixed type/position/transport categories, format counts, and bounded encoded-pixel dimensions, frame-rate ranges, autofocus, and color spaces. Limits are 32 devices, 256 formats per device, and 8,192 merged capability rows. The `NSCameraUseContinuityCameraDeviceType` classification key and a transport check both exclude Continuity Camera; Desk View is also excluded. This path requests no Camera TCC permission, opens no device, creates no input/session/output, and reads no frames, unique/model IDs, names, manufacturers, or current configuration. Results clear on page exit.
...
Camera Check is another `publicTCC` experiment, isolated from Camera Capabilities. Every live run requires two actions; only the second confirmation can request Camera permission when it is undecided. It selects only the first built-in/external local video input in the current discovery result after the same Continuity/Desk View exclusions. Preview is limited to two minutes and releases on stop, page exit, system sleep, app termination, device disconnect, or capture-session interruption/error. A 32-bit BGRA raw frame is sampled only inside its callback, at no more than 10 Hz and 4,096 regular sample points per analysis, then immediately reduced to relative digital luma and bounded frame-delivery statistics. Whole frames, pixel buffers, and sample buffers never leave the callback; there is no recording, still capture, recognition, persistence, or export. Relative luma is not lux, exposure, ambient/display brightness, or camera quality. Source, deterministic Demo, and fixtures are complete, while first authorization/denial and real-image acceptance still require the user to be present.
...
- Microphone and Camera permissions can be requested only after a second confirmation inside Sound Input Check and Camera Check respectively. This version does not request Location, Accessibility, Input Monitoring, or Full Disk Access.
- No collection of serial numbers, hardware UUIDs, user/host names, network identifiers, USB locations/paths, precise location, process lists, recordings, or camera images. Sound Input Check retains no PCM and Camera Check retains no frames; neither exports its derived values.
...
- Hardware inventory, capability semantics, and export schemas are described in [docs/07-硬件清单与能力语义.md](docs/07-硬件清单与能力语义.md). Detailed implementation status and boundaries are in [docs/05-当前实现与后续路线.md](docs/05-当前实现与后续路线.md); the Display/NVMe, IOReport, Wi-Fi, Audio, USB, and Camera implementation/design records are [docs/08-显示校准与NVMe健康可行性.md](docs/08-显示校准与NVMe健康可行性.md), [docs/09-IOReport普通权限可行性.md](docs/09-IOReport普通权限可行性.md), [docs/10-Wi-Fi无线电边界.md](docs/10-Wi-Fi无线电边界.md), [docs/11-Audio硬件清单边界.md](docs/11-Audio硬件清单边界.md), [docs/12-USB设备清单边界.md](docs/12-USB设备清单边界.md), and [docs/13-Camera硬件清单边界.md](docs/13-Camera硬件清单边界.md); the anonymous compatibility workflow is in [docs/06-匿名兼容性贡献指南.md](docs/06-匿名兼容性贡献指南.md).
