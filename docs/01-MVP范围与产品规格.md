...
- 当前值与单位；
- 原始/派生/估算/校准标记；
- 硬件领域、访问级别、兼容性证据和五段就绪状态；
- 数据来源和最后更新时间；
- 可用性、权限和错误详情。

### Experiments

首版显示可规划实验及其依赖状态。只有底层 Provider 可用且经过验证后才启用真正测量。环境光允许在保留原始通道的前提下，用用户提供的 1–8 个严格单调外部 lux 参考生成 Estimated 线性拟合，并可由用户主动撤销最后一点或导入/导出可移植校准 JSON；旧单点 JSON 保持可读。四个光谱通道可以归一化为相对比例，并与用户保存的本地参考比较相似度和最大偏移通道；由于缺少公开响应曲线和校准矩阵，不得把它包装为色温、波长光谱或 lux。上盖角度允许设置本地参考并显示相对变化。显示器物理尺允许用户以真实尺具校准窗口当前屏幕的水平轴，并把系统估算与用户校准分开展示；校准不跨 App 会话或显示模式，不推断垂直尺寸和对角线。Force Touch Pressure Lab 只在用户显式开始后读取 App 自己交互区内的公开 AppKit 事件，按 stage 分开展示 0…1 归一化 pressure 和 stageTransition；它不是力或重量测量，不使用输入监控、原始触点或设备标识，也不保存或导出样本。
...
部件温度实验可以对比固定只读 SMC 白名单中的 CPU/GPU 热点历史；系统功耗趋势可以给出近期平均值和峰值。两者都不得增加新 SMC 键或写入路径，并必须分别声明部件温度不是室温、内部功耗不是插座侧功率。
...
- 首屏不等待硬件枚举；
- Provider 独立异步加载；
- 每个 Provider 有独立的协调等待边界；单个同步读取阻塞时只降级该 Provider，且在原调用结束前不得重复启动同一读取；
- 可手动 Refresh；
...
- UI 使用英文/简体中文 String Catalog，代码、Provider/Channel ID 和导出字段使用稳定英文。
- Raw Sensors 搜索覆盖 Provider、通道、硬件领域、访问级别、兼容性证据、就绪状态和当前语言的显示名；只过滤当前显示，不得改写原始 Snapshot、采样历史或导出数据。
