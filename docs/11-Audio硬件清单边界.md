...
E7b 新增的 Sound Input Check 是与该 Provider 隔离的 `publicTCC` 面板。它仅在用户完成二次确认且授权后启动 `AVAudioEngine` input tap，并把 PCM buffer 立即缩减为受限元数据计数；当前没有任何 channel sample 访问、波形、声级、频谱、录制、播放、持久化或导出路径。App Bundle 因此现在包含经审阅的中英文 `NSMicrophoneUsageDescription` 和 Hardened Runtime Audio Input entitlement；这不改变 `hardware.audio` 的普通权限属性。
...
- PCM payload、波形、音频片段、麦克风设备身份或正在播放内容。Sound Input Check 只显示当次会话的授权/生命周期状态和无样本格式/计数元数据，并在离页时清除。
...
E7b 把 user-started PCM 作为独立 `publicTCC` 功能落地，实现边界如下：
...
- 获准后才创建 [`AVAudioEngine.inputNode`](https://developer.apple.com/documentation/avfaudio/avaudioengine/inputnode) 输入路径并安装 1024-frame tap；样本率必须在 1…768,000 Hz，通道必须在 1…64，单 buffer 必须在 1…65,536 frames；
- tap 不访问 `floatChannelData`、`int16ChannelData` 或 `AudioBufferList`，只将 frame length 和 format 变为值类型元数据。会话总数上限为 1,000,000 buffers / 500,000,000 frames，任一超限都先停流；
...
- 停止时可在面板暂留无样本计数供阅读，离页时全部清除。任何状态和元数据都不进入 Provider、Dashboard history、Snapshot、Diagnostics、JSON/CSV、UserDefaults 或日志；
...
E7c 才会在这条已验证生命周期上读取最小样本窗以计算 waveform/RMS/peak/dBFS。dBFS 与物理 dB SPL/dBA 永久分开，只有外部声级参考和独立校准设计后才能使用 Estimated/Calibrated 物理声级。FFT 在 E7d 单独评审频率分辨率、窗函数、采样率和性能上限。
...
E7b fixture 覆盖：明确二次确认前不请求权限/不启动、denied/restricted fail closed、授权请求未完成不伪称 denied、离页后迟到授权不得启动、格式改变后迟到 buffer 不得计入、五分钟上限/休眠中止、格式/启动失败、Demo 只发布有界元数据且离页清空。源码回归禁止 channel sample access、音频文件/录音 API、`tccutil` 和系统设置打开路径。发布审计另外固定中英文用途文案、单一 entitlement、资源 lint 和签名 entitlement 验收。

本机无值验证只记录 `hardware.audio` 为 Available 及其固定 Channel ID，不记录设备数量、默认设备类型、采样率或延迟值。该证据证明当前开发机路径可读，不宣称所有驱动、虚拟设备和 macOS 版本都已验证。E7b 本轮没有触发真实 TCC 提示或读取真实 PCM；首次授权、拒绝后重试、真实 buffer 和设备切换属于必须用户在场的手动验收。
