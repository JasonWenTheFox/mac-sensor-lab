...
3. 无需提权即可读取的系统、CPU/GPU 利用率、负载、内存/GPU 内存、交换空间、聚合网络吞吐量、磁盘 I/O、热状态、电池、电源、系统卷普通/重要/机会性可用容量、显示像素/逻辑尺寸与缩放倍率，以及硬件存在性信息；
...
`scripts/build-app.sh` 会在 Xcode 尚不可用时自动回退到 Command Line Tools，生成并临时签名本地 Debug `.app`；传入 `release` 会从优化后的 Release 可执行文件组装 App，并对本地 ad-hoc 签名启用、验证 Hardened Runtime。每次先在 `outputs` 同文件系统的全新暂存目录组装和验收，再替换旧的生成包，避免已删除或改名的资源残留。打包时会校验 String Catalog 与生成的简体中文 `.lproj` 一致，并拒绝在 Release 可执行文件中留下开发机的绝对用户目录构建路径。两种模式都不会修改全局开发者目录，CI 使用 Release 模式复验。
...
- Battery Trend 只使用公开电量通道的有界内存历史；仅在系统明确使用电池且未充电、至少积累 5 分钟/4 个样本并出现可测下降后，才把观察耗电率和预计耗尽时间标为 Estimated，连接电源时暂停。
- Thermal Trend 把 Foundation 的 Nominal/Fair/Serious/Critical 状态保留为 0–3 序数历史，用于显示近期最高状态与切换次数；等级间距没有物理意义，也不会伪装成温度。
- Component Thermals 对比只读 SMC 白名单中的 CPU/GPU 热点近期均值与曲线；System Power Trend 显示内部功耗的近期平均值和峰值。前者不是室温，后者不是插座侧功率。
- Network Throughput 与 Disk Activity 分别并列显示汇总收发、读写速率的近期均值和双曲线趋势；仍不采集接口、地址、设备、卷或路径身份，虚拟/隧道流量可能重复计数。
...
- Overview 在卡片前汇总加载中、可用、受限、权限不足、不可用和错误数量，并保留本次启动的状态切换总数；存在当前异常时可进入 Raw Sensors，发生过间歇性抖动时可进入 Diagnostics 查看具体 Provider。
...
| 系统电源来源 | 可用 | Apple 公开 `IOPowerSources` API；只读取供电类型、公开容量/充电状态、系统低电量告警和有效时间估算，忽略名称、ID、序列号、传输和适配器元数据 |
...
- [x] 用户接受 Xcode 许可。
- [x] 完成 Xcode 首次组件初始化并在本机通过 75 项 XCTest。
...
- [x] 对 GPU 内存和磁盘驱动 `NSNumber` 计数执行精确非负整数转换，负数或小数不会绕回成巨大的无符号读数。
