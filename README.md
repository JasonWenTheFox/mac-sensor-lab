...
1. **原始事实**：23 个独立 Provider，包括非唯一机型类别、SoC、CPU 拓扑、统一内存、Metal GPU 与 Touch ID 能力，每显示器模式/色域/EDR/估算尺寸，系统盘介质属性，以及系统性能、网络/磁盘汇总、电池/电源、热压力、只读 SMC、SPU 环境光/运动和上盖角度。
...
显示器物理尺寸在系统无法取得 EDID 时可能是 CoreGraphics 的 72 DPI 推算，因此尺寸、对角线和 PPI 始终标为 `Estimated`。存储硬件仅汇总系统卷关联整盘的固定类别和数值，不读取设备型号、序列号、UUID、卷名或路径，也不从 Disk Arbitration 猜测 SMART/NVMe 健康数据。
