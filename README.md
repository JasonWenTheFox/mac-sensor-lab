...
- 连续 CSV 分开保存机器可读的 `raw_value` 和界面使用的 `formatted_value`；文本列防止表格软件公式解释，每批先验证完整 Snapshot 契约、逐行计算 UTF-8 大小并确认整批可容纳，再以单行峰值内存逐行写入并同步；Recorder 每批重新确认真实文件末尾和大小、按 Provider 时间戳/状态去重，跨批次最多跟踪 256 个 Provider，默认达到 50 MB 前自动停止。
...
- [x] 用户接受 Xcode 许可。
- [x] 完成 Xcode 首次组件初始化并在本机通过 47 项 XCTest。
