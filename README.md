...
3. 无需提权即可读取的系统、CPU 利用率、负载、内存页分类、交换空间、聚合网络吞吐量、热状态、电池、电源、存储、显示和硬件存在性信息；
...
- 连续 CSV 分开保存机器可读的 `raw_value` 和界面使用的 `formatted_value`；每批写入后同步，默认达到 50 MB 前自动停止。
...
- [x] 用户接受 Xcode 许可。
- [x] 完成 Xcode 首次组件初始化并在本机通过 17 项 XCTest。
