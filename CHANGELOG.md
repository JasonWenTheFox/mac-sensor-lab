...
- Camera inventory remains design-only in E9a. Its proposed E9b source is limited to user-started discovery of built-in/external local cameras plus bounded resolution, frame-rate, autofocus, and color-space capabilities; unique/model IDs, names, manufacturers, Continuity/Desk View devices, authorization requests, capture inputs/sessions/outputs, frames, current activity/configuration, persistence, and exports remain excluded.
