...
- An E8b Hardware Inventory USB Device Tree that performs one user-started public IOKit read, reduces only the reviewed numeric allowlist into snapshot-local ordinals, enforces 128-device/512-interface/32-level bounds, uses deterministic Demo fixtures, and clears on page exit.
- An E9a public-API design record for a session-only Camera Capabilities inventory, separating read-only discovery and bounded format metadata from Camera TCC/capture while explicitly filtering Continuity Camera and persistent/free-text identity.
- An E9b Hardware Inventory Camera Capabilities panel with a pure bounded reducer, user-started public AVFoundation discovery, snapshot-local ordinals, built-in/external Demo fixtures, two-second presentation timeout, late-result rejection, and page-exit clearing.
- An E9c confirmation-gated Camera Check with explicit Camera TCC states, a two-minute `AVCaptureSession` lifecycle, session-only preview, deterministic Demo output, bounded frame-delivery statistics, and a pure BGRA relative-luma reducer.
...
- Sound Input Check is isolated from providers, snapshots, diagnostics, persistence, and exports. It can request microphone access only after a two-step user action and runs for at most five minutes. Raw Float32 PCM is inspected only inside the audio callback and immediately reduced to bounded derived values; samples are never retained, played, written, or exported. Spectrum work uses a reusable Accelerate DFT setup, fixed 256–2,048-frame Hann windows, at most 128 positive-frequency display bands, a −80 dB relative floor, and a 10 Hz maximum refresh cadence. dBFS remains explicitly distinct from dBA, dB SPL, and calibrated loudness; the strongest non-DC bin center is not labeled as source or pitch recognition. The bundle carries the reviewed English/Simplified Chinese microphone purpose strings and the required Hardened Runtime Audio Input entitlement.
- USB inventory reads only public numeric VID/PID, release, class/interface, current-configuration, speed, alternate-setting, and current parent-edge facts after a user action. It remains outside providers, snapshots, diagnostics, persistence, and exports. Serial/container/ECID/UDID/signature, location/path/entry ID/address, free-form names, descriptor blobs, power internals, device opens, transfers, configuration, and reset remain excluded; the release audit rejects those paths in its source.
- Camera inventory uses only user-started discovery of built-in/external local cameras plus bounded resolution, frame-rate, autofocus, and color-space capabilities. The bundle opts into distinct Continuity Camera classification and the reducer also rejects Continuity transports. Unique/model IDs, names, manufacturers, Continuity/Desk View devices, authorization requests, Camera entitlement/purpose text, capture inputs/sessions/outputs, frames, current activity/configuration, persistence, and exports remain excluded; source and release audits enforce the boundary.
- Camera Check is isolated from the inventory, providers, snapshots, diagnostics, persistence, and exports. Every live run requires two user actions; only the second may request Camera access or start capture. It chooses no preferred/default device and keeps the same fixed local-device/Continuity exclusions. Sessions stop after two minutes or on stop, page exit, sleep, termination, disconnect, interruption, runtime error, invalid frames, or safety limits. BGRA buffers are read-only and sampled at no more than 10 Hz and 4,096 pixels per analysis, then immediately reduced to bounded relative-luma and delivery statistics; frames are never copied out of the callback, recorded, photographed, recognized, written, or uploaded. Reviewed English/Simplified Chinese purpose strings and only the Audio Input and Camera entitlements are enforced by release and signature audits. First real authorization/denial and image acceptance remain pending with the user present.
